#!/usr/bin/env bash
# Run local validation for the AWS Well-Architected IaC Reviewer.
...
By default, strict mode is disabled for local convenience. Strict mode fails
when expected project paths (src, tests, examples/terraform) are missing.
